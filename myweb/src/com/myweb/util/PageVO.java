...
public class PageVO {
	...
	//기본 생성자로 생성 x, total과 Criteria를 받아서 계산하도록 선언
	
	public PageVO(int total, Criteria cri) {
		this.total = total;
		this.cri = cri; //페이징의 기준이 됨
		this.pageNum =cri.getPageNum(); //페이지번호 
		
		//끝페이지 계산 -endPage 
		//1~10 => 10  11 ~ 20 => 20
		//Math.ceil 올림 메서드...
		//계산식 = (int)Math.ceil(조회하는 페이지 번호/(double)10 버튼 갯수) *10
		this.endPage = (int)Math.ceil(cri.getPageNum()/(double)10) *10;
		// ex) 5/10 ->0.5 x 10 ->ceil 10 
