...
public class BoardController extends HttpServlet {
	...
	//.board로 끝나는 요청들은 이 컨트롤로 들어오게 설정
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/board/list.board")) {
			//나중에 변경 예정 ...왜? 게시글 목록 데이터를 전달 작업x
			//response.sendRedirect("board_list.jsp");
			
			//게시글 목록 데이터를 전달 작업 해야함.
			service = new GetListServiceimpl();
			service.execute(request, response);
			
			//GetListServiceImpl로 전달 박은 request를 강제 저장된 list를 포워드 처리
		...
		//form으로 부터 정보를 받고, UpdateServiceImple
		/*
		 * 
		 * 1.UpdateServiceImpl을 생성
		 * 2. 서비스 영역에서 num,title,content을 받아서 update()메서드(DAO) 실행
		 * 3.DAO에서 update()메서드 생성.  SQL update구문을 사용하여 데이터 수정
		 * 4.페이지 이름이 상세보기인 화면 (content.board)으로 연결
		 * 	(주의 사항, 필요한 값을 전달해야함)
		 * 
